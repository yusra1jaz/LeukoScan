...
import { Eye, EyeOff, Camera, Mail, Phone, Calendar, MapPin, Award, Briefcase, User, Settings, Lock } from "lucide-react";
...
const DoctorSettings: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDoctor = async () => {
      try {
        const res = await fetch(`http://localhost:4000/api/doctors/profile`, {
          credentials: "include",
        });
  ...
  const handleChange = (
    ...
    section: "personal" | "professional" | "security"
  ) => {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      // Update local doctor object so UI reflects new password if present
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Doctor Settings</h1>
            </div>
            <div className="flex items-center space-x-2">
              <div className="text-right">
                <User className="h-8 w-8 text-blue-600 dark:text-blue-400 ml-auto mb-1" />
                <p className="text-sm text-gray-600 dark:text-gray-300 font-medium">Dr. {doctor.name || 'Unknown'}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center space-x-6 pb-6 border-b border-gray-200 dark:border-gray-700">
                  <div className="relative">
                    {doctor.photo ? (
                      <img
                        src={typeof doctor.photo === "string" ? `http://localhost:4000${doctor.photo}` : URL.createObjectURL(doctor.photo)}
                        alt="Profile"
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (file) setDoctor(prev => ({ ...prev, photo: file }));
                        }}
                        className="hidden"
                      />
                    </label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleChange(e, "professional")}
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors"
                      placeholder="Your medical degree"
                    />
                  </div>
